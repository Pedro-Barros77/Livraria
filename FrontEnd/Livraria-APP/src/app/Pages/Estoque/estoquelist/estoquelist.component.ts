...
@Component({
  selector: 'estoques-list',
  templateUrl: './estoquelist.component.html',
  styleUrls: ['./estoquelist.component.css']
})
export class EstoquelistComponent implements OnInit {
  ...
  modalDeleteSingle(estoque: Estoque) {
    ...
    delModal.setEstoque(estoque);
  }

  modalDeleteMultiple(ids: number[]) {
    if (ids.length > 0) {
      let delModal: DeleteModalComponent = new DeleteModalComponent(this.service);
      delModal.setIdEstoques(ids);
